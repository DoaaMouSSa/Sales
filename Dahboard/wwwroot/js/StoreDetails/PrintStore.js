	...
	function print_element($element_id = null) {
		if ($element_id == null) {
			...
			var link = '<link href="../css/mainCss.css") rel="stylesheet" />';
			...
			new_html.find('main').append("<h1 class='text-center'>تقرير الاصناف والمخزون</h1><br>")
			new_html.find('main').append(el)
			console.log(el);
			// Opening a new window (Pop-up) for the print view
			var new_window = window.open('', '', 'location=no,width=1200,height=700,left=250')
