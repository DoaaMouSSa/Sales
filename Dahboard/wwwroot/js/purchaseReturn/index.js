...
function getAllPurchase() {
    ...
    if ($("#chboxAllPurchase").prop('checked', true)) {
        $.ajax({
            url: "https://localhost:44315/api/PurchaseReturn/ReadAllPurchaseReturnInvoices",
            type: 'GET',
            beforeSend: function () {
            ...
            success: function (result) {
                ...
                for (var i = 0; i < result.payload.length; i++) {
                    ...
                    if (row.isDeleted == 0) {
                        $('#purchase_data').append("<tr><td onclick=window.location='Purchase/PurchaseInvoiceDetails/" + row.id + "'>" + row.id + "</td><td>" + row.supplier_name + "</td><td>" + row.store_name + "</td><td>" + row.invoice_total + "</td><td class='0'><span class='label label-success'>معتمد</span></td><td>" + row.purchase_Added_Time_ar + "</td><td><i class='fa fa-remove hand red' id=" + row.id + " data-toggle='modal' data-target='#DeleteModal" + row.id + "'></i></td></tr>");
                        ...
                        $('#purchase_data').append("<tr onclick=window.location='Purchase/PurchaseInvoiceDetails/" + row.id + "'><td>" + row.id + "</td><td>" + row.supplier_name + "</td><td>" + row.store_name + "</td><td>" + row.invoice_total + "</td><td class='1'><span class='label label-danger'>غير معتمد</span></td><td>" + row.purchase_Added_Time_ar + "</td><td><i class='fa fa-remove hidden'></i></td></tr>");
