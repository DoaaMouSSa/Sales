...
function getProductInfo(txtId, id, name, price) {
    var numOftr = $("#return_salesTbl tr").length;
    if (numOftr == 2) {
        $("#" + txtId + "").val(id);
...
function CalculateSalesinvoiceForOneProduct(idRow) {
    ...
    var t = document.getElementById('return_salesTblBody');
...
function DeleteRow(idRow) {
    var rowsCount = $('#return_salesTbl tbody').find('tr').length;
    if (rowsCount > 1) {
        var index = $(idRow).closest('tr').index();
        index = index + 1;
        document.getElementById("return_salesTbl").deleteRow(index);
        getTotal();
    }
}
function getTotal() {
    ...
    $("#Be4DiscountNumberReturnSale").val(sum)
}